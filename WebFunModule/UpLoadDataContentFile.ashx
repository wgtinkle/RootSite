<%@ WebHandler Language="C#" Class="UpLoadDataContentFile" %>
...
public class UpLoadDataContentFile : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        try
        {
            string sTitle = context.Request.Form["DataTitle"];
            string sTag = context.Request.Form["DataTag"];
            string sContent = context.Request.Form["DContent"];
            string sImg = context.Request.Form["DataImg"];
            string sPassport = context.Request.Form["PName"];
            

            if (sTitle == "")
            {
                sResult = "UpLoadDataTitleNull";
            }
            else if (sTag == "")
            {
                sResult = "UpLoadDataTagNull";
            }
            else if (sContent == "")
            {
                sResult = "UpLoadDataContentNull";
            }
            else
            {
                ...
                reader.Close();//释放读写器
                query = "select max(dataid) from dballwithme.datainfo";
                ...
                Maxreader.Close();
                mxaid++;
                string sFileName = "datacontent" + mxaid.ToString() + ".htm";
                string sTime = DateTime.Now.ToString("yyyy/MM/dd");
                int nCollect = 0;
                query = "insert into dballwithme.datainfo values(" + mxaid.ToString() + ","
                    + "'" + sTitle + "'" + ","
                    + "'" + "简单说明" + "'" + ","
                    + "'" + sImg + "'" + ","
                    + "'" + sFileName + "'" + ","
                ...
                hdom.Save(siteRecordfilepath + "PageData\\" + sFileName, Encoding.UTF8);
                sResult = "UpLoadDataOk";
            }
        }
        catch (Exception e)
        {
